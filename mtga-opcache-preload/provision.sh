...
if [[ ! -f "/etc/php/8.2/fpm/conf.d/20-opcache.ini" ]]; then
  ...
  sudo "opcache.enable=1" >> /etc/php/8.2/fpm/conf.d/20-opcache.ini
  sudo "opcache.preload_user=www-data" >> /etc/php/8.2/fpm/conf.d/20-opcache.ini
  sudo "opcache.preload=/srv/www/mtga/public_html/preload.php" >> /etc/php/8.2/fpm/conf.d/20-opcache.ini
else
